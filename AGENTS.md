...
The charged-world ZIP check is `node tools/verify_storage_world.mjs <world.zip> <fixture.json> data/statech-2.0.1.json.gz <report.json>`. Preserve exact saved `storedEu` as a quantity; the current recipe-correction prompt for storage units is still an open importer defect under #41.
