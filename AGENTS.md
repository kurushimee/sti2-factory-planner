...
Saved solar panels keep origin, cell, and fluid facts in `reconstruction.solar_panels`. Import their enabled routes as exact `request.installed` counts without creating power goals or treating saved stocks as recurring supplies. Keep route and enabled corrections editable, preserve graph positions on route changes, and show inferred storage as planned support. Verify the controlled private archive with `node tools/verify_solar_world.mjs <world.zip> <solar-panels.json> data/statech-2.0.1.json.gz data/provenance/solar-world-report.json`, `tools/test_solar_world_ui.gd`, and `tools/verify_solar_desktop.mjs` and `tools/verify_solar_world_app_browser.mjs` against exported builds. For exact solar capacity goals, preserve zero-machine and original-unit checks during any numerical retry. Check `kernel/solar_capacity.test.js`, `tools/verify_solar_capacity.mjs`, `tools/test_solar_capacity_ui.gd`, browser Worker parity with `--solar`, and `tools/verify_solar_capacity_app_browser.mjs` against the extracted web package; recovered plans must remain unproven and show their roundoff bound.
