...
Color Picker ink routes use the loaded recipes and controlled trial in `data/provenance/spectrum-ink-report.json`. Regenerate them with `tools/extraction/add_spectrum_ink_routes.py`, then run `node tools/verify_spectrum_ink_catalog.mjs data/statech-2.0.1.json.gz` and the browser Worker check with `--ink`. The follow-up `data/provenance/spectrum-growth-report.json` verifies loaded ink-node transfer, stage-gated pickup, and all 20 loaded cluster loot rules. An always-on AE2 plane breaks immature buds. `data/provenance/turtle-growth-report.json` records a controlled turtle farm with direct commands. `data/provenance/turtle-lua-report.json` records two iron harvests with a real Lua program in an unpaused world; reproduce them with `run_capture.py --turtle-lua` and `verify_turtle_lua.py` using the private inputs in the extraction guide. Keep passive Crystallarieum routes unsupported until all relevant recipes, support chains, throughput limits, and construction costs are modeled and checked.
