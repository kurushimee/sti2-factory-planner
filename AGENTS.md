...
Keep construction quantities separate from sustained flow rates. The optional construction-cost model uses continuous material equivalents and assumes reusable construction workstations; preserve these visible assumptions until finite crafting and bootstrap are implemented. Check it with `node tools/verify_construction_catalog.mjs`, `tools/test_construction_ui.gd`, and the exported `node tools/verify_construction_browser.mjs` interaction check.
