...
Saved solar panels keep origin, cell, and fluid facts in `reconstruction.solar_panels`. Import their enabled routes as exact `request.installed` counts without creating power goals or treating saved stocks as recurring supplies. Keep route and enabled corrections editable, preserve graph positions on route changes, and show inferred storage as planned support. Verify the controlled private archive with `node tools/verify_solar_world.mjs <world.zip> <solar-panels.json> data/statech-2.0.1.json.gz data/provenance/solar-world-report.json`, `tools/test_solar_world_ui.gd`, and `tools/verify_solar_desktop.mjs` and `tools/verify_solar_world_app_browser.mjs` against exported builds. The three water-assisted exact capacity goals expose a numerical infeasibility tracked in issue #54; do not treat that case as verified.
