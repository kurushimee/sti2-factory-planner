...
Keep periodic generation separate from the existing steady energy balance until storage, replacement fuel, construction, and topology assumptions are integrated. The fixed-profile checks are `node --test kernel/periodic_power.test.js kernel/solar_profile.test.js`; the matching loaded storage capture is verified with `tools/extraction/verify_storage.py`. A daily solar average alone is not firm power. Issue #37 tracks the remaining integration and export checks.

The charged-world ZIP check is `node tools/verify_storage_world.mjs <world.zip> <fixture.json> data/statech-2.0.1.json.gz <report.json>`. Preserve exact saved `storedEu` as a quantity in `reconstruction.storage_units`. Storage has no recipe or output goal. It is not yet carried into the power plan; #41 and #37 track that integration. Regenerate the bundled rules through `merge_storage_rules.py`, `player_dataset.py`, and `bundle_dataset.py`, retaining the verified rotation capture and the storage report hash.
