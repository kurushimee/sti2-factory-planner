...
The optional `progression` list supplies named availability presets. Each entry needs unique `id`, `name`, `available_machines`, and `available_upgrades` fields. Machine references may name a machine record or a fixed configuration's machine. `description` and `source` explain the choice. Applying a preset replaces those two availability lists and records `request.progression_preset`; individual overrides remain in the lists. It does not create external supplies, mark items obtained, alter replication mode, or discard route pins.
