...
An energy-storage machine uses `status: "infrastructure"`, `mechanic: "energy_storage"`, and a `storage` object. The object needs positive whole `capacity_eu`, `charge_eu_per_tick`, and `discharge_eu_per_tick`, a nonnegative whole `loss_eu_per_tick`, and the saved NBT field name in `saved_charge_field`. The importer keeps the exact saved charge as decimal text with its world origin and rejects missing or out-of-range charge. This is startup state, not an energy-generating recipe. The current periodic calculator accepts lossless fixed profiles; the connected factory planner does not yet install or dispatch storage.
