...
The Imported factory dialog shows machine origins, saved upgrades, recipe evidence, and compatible recipe choices. Applying corrections recalculates inferred capacity goals and preserves unresolved entries. The real-catalog browser check also covers undo across dataset changes and removing the final imported goal. Large-catalog performance and the remaining goal/configuration controls still need work.
