...
After exporting the browser application, run `node tools/verify_app_browser.mjs`. It operates the real canvas controls inside a cross-origin iframe and checks goal creation, group movement, boundary resizing without node movement, portable plan import, undo/redo, malformed-file recovery, download, and persistence across reload. Optional arguments `<world.zip> <machines.json>` also exercise the actual world ZIP through the application's file picker. The fixture remains private.
