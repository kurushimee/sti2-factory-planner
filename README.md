...
The calculation kernel uses the same HiGHS WebAssembly build in Node and a browser Worker. The Godot workspace supports goals, resource connections, inspection, movable and resizable groups, undo/redo, and portable plans. Clear-weather solar routes use measured MI storage limits, cell and water supplies, and an explicit initial-charge requirement. Imported MI storage units retain their saved charge and origins, set editable installed counts, and appear in the power graph without creating a production goal. The world reader recovers tested MI configuration and both AE2 provider forms, but complete production-line reconstruction is unfinished.
