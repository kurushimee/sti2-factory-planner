...
The calculation kernel uses the same HiGHS WebAssembly build in Node and a browser Worker. The Godot workspace supports goals, resource connections, inspection, movable and resizable groups, undo/redo, and portable plans. New plans start in production-only mode with external electricity; generation optimization and complete world reconstruction remain outside the preview acceptance gate. Existing solar, storage, and import work stays available with its recorded limits.
