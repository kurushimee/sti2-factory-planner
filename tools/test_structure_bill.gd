...
func _run() -> void:
	...
	for value: Dictionary in workspace._dataset.recipes:
		if (irradiation_mode && value.id == "irradiate|item:modern_industrialization:uranium_fuel_rod|modern_industrialization:beryllium_block") || (!irradiation_mode && value.primary == "item:modern_industrialization:steel_ingot" && value.get("process", {}).get("type") == "modern_industrialization:blast_furnace"):
			recipe = value
			break
	...
	if irradiation_mode:
		...
		workspace._request.available_machines = ["yet_another_industrialization:nuclear_rod_irradiator"]
	...
	if irradiation_mode:
		...
		assert(settings._matches.any(func(entry: Dictionary) -> bool: return entry.id == "modern_industrialization:nuclear_item_hatch" && !entry.unsupported))
		if DisplayServer.get_name() != "headless":
			await create_timer(0.2).timeout
			await RenderingServer.frame_post_draw
			root.get_texture().get_image().save_png("res://.plans/artifacts/workspace/irradiation-hatch-settings.png")
