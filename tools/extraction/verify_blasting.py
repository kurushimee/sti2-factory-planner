...
def verify(base, normalized, runtime, spectrum_jar, fixture, capture, world_archive, probe_jar):
    selected = json.loads(fixture.read_text(encoding="utf-8"))
    if selected != distinct_blasting(json.loads(runtime.read_text(encoding="utf-8"))):
        raise ValueError("The prepared blasting fixture differs from the loaded recipes.")
    if sha256(spectrum_jar) != SPECTRUM_JAR_SHA256:
        raise ValueError("The installed Spectrum jar differs from the released pack input.")
    ...
    return {
        ...
        "loaded_runtime_sha256": sha256(runtime),
        "initial_pack_runtime_sha256": "521777580a030c3fa2b49b007062243512e4e4299d7854e4969d123b19316d8b",
        "base_catalog_sha256": sha256(base),
        ...
        "limits": [
            ...
            "The lava bucket was tick-tested with pure iron. Other lava pairings use the loaded furnace's shared fuel method but were not individually tick-tested.",
...
if __name__ == "__main__":
    ...
    for name in ("base", "normalized", "runtime", "spectrum_jar", "fixture", "capture", "world_archive", "probe_jar", "output"):
        parser.add_argument("--" + name.replace("_", "-"), type=Path, required=True)
    args = parser.parse_args()
    report = verify(args.base, args.normalized, args.runtime, args.spectrum_jar,
                    args.fixture, args.capture, args.world_archive, args.probe_jar)
