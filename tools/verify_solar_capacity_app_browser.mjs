...
if (!planPath) throw new Error('Supply the checked portable solar capacity plan.');
const root = resolve(process.env.STI2_WEB_ROOT ?? 'builds/web');
const artifacts = resolve('.plans/artifacts/solar-capacity');
...
try {
  ...
  for (let attempt = 0; attempt < 600; attempt++) {
    ...
    if (saved?.request?.goals?.length === 3 &&
        Object.keys(saved.positions ?? {}).filter(key => key.startsWith('planner:solar|')).length === 3) break;
    await new Promise(done => setTimeout(done, 100));
  }
  assert.equal(saved?.request?.goals?.length, 3);
  assert.ok(saved.request.goals.every(goal => goal.kind === 'capacity' && goal.machines === 1));
  assert.equal(Object.keys(saved.positions).filter(key => key.startsWith('planner:solar|')).length, 3);
  await page.mouse.click(996, 92, {delay: 100});
  await page.screenshot({path: `${artifacts}/browser-capacity-plan.png`});
  ...
  assert.equal(portable.request.goals.length, 3);
  assert.equal(Object.keys(portable.positions).filter(key => key.startsWith('planner:solar|')).length, 3);
  await page.reload();
  ...
  console.log('The embedded application planned, exported, and restored three exact solar capacity goals.');
} catch (error) {
  await page?.screenshot({path: `${artifacts}/browser-capacity-failure.png`});
  throw error;
} finally {
  await browser?.close();
